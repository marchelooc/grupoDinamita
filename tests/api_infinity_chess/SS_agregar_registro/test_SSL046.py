...
def test_registro_codigo_duplicado (get_url):
     logger.info("Iniciando test SSL046.")
     ...
     response2 = enviar_solicitud (get_url,payload)
     logger.info(f"Código de respuesta: {response2.status_code}.")
     assert response2.status_code == 409
     logger.debug(f"Response: {response2.json()}")
     logger.info("Validando schema del response.")
     assert_validar_response_schema(response2,cargar_schema("schema_registro.json"))
