...
from src.utils.payload.payload_registro import crear_payload_sin_codigo_trabajador

@pytest.mark.negative
@pytest.mark.xfail(reason="Knwon issue SSBUG006: Sistema no valida datos del body",run=True)
def test_registro_sin_codigo_trabajador (get_url):
     logger.info("Iniciando test SSL042.")
     payload = crear_payload_sin_codigo_trabajador(get_url)
