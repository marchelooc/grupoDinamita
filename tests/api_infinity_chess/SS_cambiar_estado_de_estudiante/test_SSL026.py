...
def test_validación_de_valor_vacio_en_campo_estado (get_url):
     ...
     logger.debug(f"Payload: {payload_vacioH}")
     logger.info("Validando schema del payload.")
     assert_validar_schema_input(payload_vacioH, cargar_schema("schema_habilitado.json"))
     response = enviar_solicitud (get_url, CODESTUDIANTE,payload_vacioH)
